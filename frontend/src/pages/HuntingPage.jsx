...
const HuntingPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      target_zip: formData.target_zip,
      max_distance_miles: parseInt(formData.max_distance_miles) || 200,
      is_active: true
    };
  ...
  return (
    <div className="hunting-container">
      ...
      {/* 📡 AUTOMATED RADAR CADENCE BANNER */}
      <div style={{ 
        maxWidth: '600px', 
        width: '100%',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          <span style={{ color: '#ff9900', fontWeight: 'bold', fontSize: '0.9rem' }}>📡 Background Scraper Radar:</span>
          <span style={{ color: '#bdc3c7', fontSize: '0.8rem' }}>Automated search footprints scan the market every 30 minutes.</span>
        </div>
        <span style={{ 
          fontSize: '0.75rem', 
      ...
      <h3>Enter Vehicle Search Criteria</h3>
      ...
      <form onSubmit={handleSubmit} className="hunting-form">
        ...
        <div className="input-wrapper">
          ...
          <input type="text" name="target_zip" value={formData.target_zip} onChange={handleInputChange} className="hunting-input" placeholder="e.g. 22901"/>
        </div>

        <div className="input-wrapper">
          <label>Search Radius (Miles)</label>
          <input type="number" name="max_distance_miles" value={formData.max_distance_miles} onChange={handleInputChange} className="hunting-input" placeholder="e.g. 200"/>
        </div>
      </div> 
      ...
      {/* 🧭 NAVIGATION TOOLBAR (NOW CENTERED & CLEAN) */}
